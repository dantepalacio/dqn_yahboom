...
#         print(pixel_shape)
#         print('DQN VXOD VECTOR ',vector_shape)
...
#         self.model = self.build_model(pixel_shape,vector_shape)
#         self.target_model = self.build_model(pixel_shape,vector_shape)
...
#         return model
#     def build_model(self,pixel_shape, vector_shape):
#         # Вход для матрицы пикселей
#         pixel_input = tf.keras.Input(shape=pixel_shape)

#         # Вход для вектора
#         vector_input = tf.keras.Input(shape=vector_shape)
...
#         return model
    ...
    def build_model(self):
        ...
        # Создание модели с входом для матрицы пикселей и одним выходом
        model = tf.keras.Model(inputs=pixel_input, outputs=output)
        model.compile(optimizer='adam', loss='mse')
        return model
    ...
    def train(self, state, action, reward, next_state, done, verbose=True):
        ...
        self.model.fit(state, target, epochs=2, verbose=verbose)
...
def distance_check():
    ...
    print('DISTANCE SHAPE: ', state.shape)
    return state

def frame_check(image):
    image = cv2.resize(image, (100, 100))
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    print('KARTINKA RAZMER ',image.shape)
...
action_dim = 3  # Specify the number of possible actions
# pixel_shape = (100,100)
# vector_shape = (2,)
...
try:
    while True:
        ...
        img = np.expand_dims(img, axis=0)  # Изменение формы img на (1, 100, 100)
        ...
        print('ASOHAUIDAIS ',state)
        print('ASOKASPWQWQ', len(state))
        print('ASIJAIDJIWQ', state[1])
        print('ASKAKAOPDOQWJI', state[0].shape)
        print('ASKAKAOPDOQWJI', state[1].shape)
