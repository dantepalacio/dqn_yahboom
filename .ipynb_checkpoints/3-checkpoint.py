...
class DQN:
    ...
    def train(self, state, action, reward, next_state, done, verbose=True):
        ...
        print('asdojaqwieojiods', pixel_state.shape)
        print('asdojaqwieojiods', vector_state.shape)

        target = self.model.predict([pixel_state, vector_state], batch_size=1)
        print('TARGEEEEEEEEET', target)
        ...
        self.model.fit([pixel_state, vector_state], target, epochs=5, verbose=verbose)
